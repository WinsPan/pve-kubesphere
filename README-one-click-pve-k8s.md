...
- 🔧 **智能诊断**：内置诊断功能，快速定位问题
- 📦 **无人值守**：使用Debian cloud镜像和cloud-init自动配置系统
- 🌐 **多源下载**：支持多个镜像源，提高下载成功率
- 🔄 **自动重试**：关键步骤支持自动重试机制
- 📊 **详细日志**：完整的部署日志记录
- 🎯 **交互菜单**：友好的菜单界面，操作简单
- 🔄 **双重方案**：优先使用cloud镜像，失败时回退到ISO方案
...
### Cloud镜像方式（推荐）
1. **环境检查**：检查PVE环境和依赖工具
2. **镜像下载**：下载Debian cloud镜像
3. **虚拟机创建**：导入cloud镜像创建虚拟机
4. **Cloud-init配置**：配置网络、用户、密码
5. **系统启动**：批量启动虚拟机
6. **SSH等待**：等待虚拟机SSH服务可用
7. **系统初始化**：安装基础软件包
8. **K8S部署**：安装Kubernetes集群
9. **KubeSphere安装**：部署KubeSphere控制台

### ISO方式（备用）
1. **ISO下载**：下载Debian netinst ISO
2. **虚拟机创建**：创建虚拟机并挂载ISO
3. **手动安装**：在虚拟机控制台完成系统安装
4. **系统配置**：设置网络、密码等
5. **继续部署**：执行K8S和KubeSphere安装
...
**解决**：
- 脚本会自动回退到ISO方式
- 手动下载镜像到 `/var/lib/vz/template/qcow/`

### 3. SSH连接失败
**原因**：cloud-init未生效或密码错误
**解决**：
- 检查虚拟机配置：`qm config 101`
- 重新配置cloud-init：`qm set 101 --ciuser root --cipassword kubesphere123`
...
7. Cloud镜像方式完全自动化，推荐使用
8. ISO方式需要手动干预，仅作为备用方案
